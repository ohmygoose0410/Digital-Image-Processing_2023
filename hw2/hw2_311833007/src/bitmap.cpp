...
void HSL::fromRGB (const Pixel & pixel) {
	float r = (float)pixel.red / 255;
	float g = (float)pixel.green / 255;
	float b = (float)pixel.blue / 255;
...
Pixel HSL::toRGB() {
	...
	if (saturation == 0) {
		pixel.red = pixel.green = pixel.blue = lightness * 255;		
	}
	else {
		float q = lightness < 0.5 ? lightness * (1 + saturation) : lightness + saturation - lightness * saturation;
		float p = 2 * lightness - q;

		pixel.red 	= hue2rgb(p, q, hue + 1./3) * 255;
		pixel.green = hue2rgb(p, q, hue) * 255;
		pixel.blue 	= hue2rgb(p, q, hue - 1./3) * 255;
	}
