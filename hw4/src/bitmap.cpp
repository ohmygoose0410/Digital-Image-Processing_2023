			...
			for (int row = 0; row < dib_header.height; row++) {
				...
				if (existalpha) {
					for (int col = 0; col < dib_header.width; col++) {
						...
						row_data.push_back(Pixel<int>(red, green, blue, alpha));
					}
				}
				else {
					for (int col = 0; col < dib_header.width; col++) {
...
void BMP::Write(std::string filename, bool save_alpha) {
	...
	if (file.fail()) {
		...
		for (int row = pixels.size() - 1; row >= 0; row--) {
			const PixelVector & row_data = pixels[row];

			if (save_alpha) {
				for (int col = 0; col < row_data.size(); col++) {
					const Pixel<int>& pix = row_data[col];

					file.put((uchar_t)(pix.c3));
					file.put((uchar_t)(pix.c2));
					file.put((uchar_t)(pix.c1));
					file.put((uchar_t)(pix.c4));
				}
			}
			else {
				for (int col = 0; col < row_data.size(); col++) {
...
bool BMP::ValidImage() {
	...
	for(int row=0; row < height; row++) {
		...
		for(int col=0; col < width; col++) {
			Pixel<int> pix = pixels[row][col];
			if( pix.c1 > MAX_VAL || pix.c1 < MIN_VAL ||
				pix.c2 > MAX_VAL || pix.c2 < MIN_VAL ||
				pix.c3 > MAX_VAL || pix.c3 < MIN_VAL ||
				pix.c4 > MAX_VAL || pix.c4 < MIN_VAL) {
				std::cerr << "Wrong row, col: " << row << "," << col << "\n";
