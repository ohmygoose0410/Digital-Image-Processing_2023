		...
		for (int row = pixels.size() - 1; row >= 0; row--) {
			...
			if (save_alpha) {
				for (int col = 0; col < row_data.size(); col++) {
					...
					file.put((int)(pix.blue));
					file.put((int)(pix.green));
					file.put((int)(pix.red));
					file.put((int)(pix.alpha));
				}
			}
			else {
				for (int col = 0; col < row_data.size(); col++) {
...
bool BMP::ValidImage() {
	...
	for(int row=0; row < height; row++) {
		...
		for(int col=0; col < width; col++) {
			...
			if( pix.red > cMAX_VAL || pix.red < MIN_VAL ||
				pix.green > cMAX_VAL || pix.green < MIN_VAL ||
				pix.blue > cMAX_VAL || pix.blue < MIN_VAL ||
				pix.alpha > cMAX_VAL || pix.alpha < MIN_VAL) {
