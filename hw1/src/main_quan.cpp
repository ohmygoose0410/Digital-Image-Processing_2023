...
using namespace std;
// bits輸入要降解析的目標bit值
PixelMatrix quantization(const PixelMatrix & org_img, const int bits) {
  PixelMatrix quan_img = org_img;
  // 產生bitmaskt -> 2^8 - 2^(8 - bits)
  int bitmask = pow(2, 8) - pow(2, (8 - bits));

  for(int row = org_img.size() - 1; row >= 0; row--) {
    vector <Pixel> & row_data = quan_img[row];

    for(int col = 0; col < row_data.size(); col++) {
      Pixel & pix = row_data[col];
  ...
  for (int cnt = 1; cnt < argc; cnt++) {
    ...
    if(validBmp == true) {
      ...
      for (int level = 0; level != (sizeof(levels)/sizeof(levels[0])); level++) {
        ...
        quan_image.FromPixelMatrix(quan_bmp);
